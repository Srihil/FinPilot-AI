# -*- coding: utf-8 -*-
"""
Full extraction test suite for DemoEntityAgent.

Covers ALL entity types the AI Create page supports:
  - 8 accounting voucher types
  - 6 stock transaction types
  - 8 inventory / accounting master types
    (unit, godown, stock_item, stock_group, ledger, group, customer, vendor)
  - 3 custom voucher types (GST Bill, Petty Cash, FP-Sales)

Run:  python -X utf8 backend/tests/test_entity_extraction.py
Exit: 0 = all passed,  1 = one or more failures  (used by pre-commit hook)
"""
import sys, os, io
...
# ---------------------------------------------------------------------------
# Schema: (expected_type, text, check_key, expected_value_contains, date_or_None)
#   date_or_None = None  -> skip date check (master entities have no date)
#   check_key = 'entity_type_self'  -> check entity_type field itself
# ---------------------------------------------------------------------------
TESTS = [

    # ==========================================================================
    # ACCOUNTING VOUCHERS  (8 types)
    # ==========================================================================
    ...
    # Petty Cash used as ACCOUNT (must stay contra, not custom_voucher)
    ('contra',
     f'Bank transfer {RS}50000 from Petty Cash to SBI Current Account on 1 Sept 2026',
     'from_account', 'Petty Cash', '2026-09-01'),
    ('contra',
     f'Transfer {RS}20000 from Petty Cash to HDFC Bank on 15 Aug 2026',
    ...
    # ==========================================================================
    # CUSTOM VOUCHER TYPES  (user has: GST Bill, Petty Cash, FP-Sales)
    # ==========================================================================
    ...
    # Petty Cash as VOUCHER TYPE (not account)
    ('custom_voucher',
    ...
    # Negative: Petty Cash as ACCOUNT must not route to custom_voucher
    ('contra',
    ...
    # --- UNIT (supported via "Create Unit of measure" chip) -------------------
    ...
    ('unit', 'Add unit Pieces symbol Pcs decimal 2',
     'symbol', 'Pcs', None),
    ('unit', 'Create unit Bags symbol Bag',
     'name', 'Bags', None),
    ('unit', 'Add unit Litres symbol Ltr decimal 2',
     'name', 'Litres', None),
    ...
    ('godown', 'Add Godown Mumbai under Main Location',
     'name', 'Mumbai', None),
    ('godown', 'Add Godown Mumbai under Main Location',
     'parent', 'Main Location', None),
    ('godown', 'Create godown Delhi Branch under Main Location',
     'name', 'Delhi Branch', None),
    ...
    ('stock_item', f'Add stock item Samsung Galaxy unit Nos rate 25000',
     'rate', '25000', None),
    ('stock_item', 'Add product Office Chair group Furniture unit Nos rate 5000 opening qty 10',
     'name', 'Office Chair', None),
    ('stock_item', 'Add product Office Chair group Furniture unit Nos rate 5000 opening qty 10',
     'opening_qty', '10', None),
    ('stock_item', 'Add product Office Chair group Furniture unit Nos rate 5000 opening qty 10',
     'stock_group', 'Furniture', None),
    ('stock_item', f'Create stock item Laptop unit Nos rate 45000 opening qty 5',
     'name', 'Laptop', None),
    ('stock_item', f'Create stock item Laptop unit Nos rate 45000 opening qty 5',
     'rate', '45000', None),
    ...
    ('stock_group', 'Create item group Furniture',
     'name', 'Furniture', None),
    ('stock_group', 'Add stock group Mobiles under Electronics',
     'name', 'Mobiles', None),
    ('stock_group', 'Add stock group Mobiles under Electronics',
     'parent', 'Electronics', None),
    ('stock_group', 'Create stock group Appliances',
    ...
    ('group', 'Create account group Fixed Assets under Capital Account',
     'name', 'Fixed Assets', None),
    ('group', 'Add group Marketing Expenses under Indirect Expenses',
     'name', 'Marketing Expenses', None),
    ('group', 'Add group Marketing Expenses under Indirect Expenses',
     'parent', 'Indirect Expenses', None),
    ...
    ('ledger', 'Create ledger Sales under Direct Income',
     'name', 'Sales', None),
    ('ledger', 'Add ledger Kumar Enterprises under Sundry Debtors',
     'name', 'Kumar Enterprises', None),
    ('ledger', 'Add ledger Kumar Enterprises under Sundry Debtors',
     'group', 'Sundry Debtors', None),
    ('ledger', 'Create ledger Petty Cash under Cash-in-Hand',
     'name', 'Petty Cash', None),
    ('ledger', 'Create ledger Petty Cash under Cash-in-Hand',
     'group', 'Cash-in-Hand', None),
    ...
    ('customer',
     'Add customer ABC Corp GST 27AABCU9603R1ZX phone 9876543210',
    ...
    ('vendor',
     ...
     'gstin', '27AABCU9603R1ZX', None),
    ('vendor',
     'Add supplier Global Tech Ltd email info@globaltech.com',
     'name', 'Global Tech Ltd', None),
]
...
SECTIONS = {
    ...
    'unit':          'MASTER - UNITS',
    'godown':        'MASTER - GODOWNS',
    'stock_item':    'MASTER - STOCK ITEMS',
    'stock_group':   'MASTER - STOCK GROUPS',
    'group':         'MASTER - ACCOUNT GROUPS',
...
for exp_type, text, key, exp_val, exp_date in TESTS:
    ...
    val = et if key == 'entity_type_self' else str(d.get(key, ''))
...
total = passed + failed
print(f'\n{"=" * 60}')
print(f'  RESULT: {passed} PASSED   {failed} FAILED   out of {total}')
print(f'{"=" * 60}\n')
