...
import type { TallyGodown } from '../../types';
...
export default function GodownsPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast({ title: 'Godown created' }); qc.invalidateQueries({ queryKey: ['godowns'] }); setShowCreate(false); setFormName(''); setFormParent(''); },
  ...
  const updateMut = useMutation({
    mutationFn: () => managementApi.updateGodown(editItem!.id, { name: formName || undefined, parent: formParent || undefined }),
    onSuccess: () => { toast({ title: 'Updated' }); qc.invalidateQueries({ queryKey: ['godowns'] }); setEditItem(null); },
    onError: (e: { response?: { data?: { detail?: string } } }) => { toast({ title: 'Error', description: e.response?.data?.detail || 'Failed', variant: 'destructive' }); },
  });

  const deleteMut = useMutation({
    mutationFn: () => managementApi.deleteGodown(deleteItem!.id),
    onSuccess: (res: { status?: string; message?: string }) => { toast({ title: res.status === 'pending' ? 'Delete queued' : 'Deleted', description: res.message }); qc.invalidateQueries({ queryKey: ['godowns'] }); setDeleteItem(null); },
    onError: (e: { response?: { data?: { detail?: string } } }) => { toast({ title: 'Error', description: e.response?.data?.detail || 'Failed', variant: 'destructive' }); },
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Warehouse className="w-6 h-6 text-indigo-600" /> Godowns</h1><p className="text-sm text-slate-500 mt-0.5">Warehouses and storage locations synced with TallyPrime</p></div>
        <Button onClick={() => { setFormName(''); setFormParent(''); setShowCreate(true); }} className="gap-2"><Plus className="w-4 h-4" /> New Godown</Button>
      </div>
      <div className="relative max-w-sm"><Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" /><Input placeholder="Search godowns…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="pl-9" /></div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>New Godown</DialogTitle></DialogHeader>
        <div className="space-y-3"><div><Label>Name *</Label><Input value={formName} onChange={e=>setFormName(e.target.value)} placeholder="e.g. Main Warehouse"/></div><div><Label>Parent Location</Label><Input value={formParent} onChange={e=>setFormParent(e.target.value)} placeholder="Leave empty for root"/></div></div>
      ...
      <Dialog open={!!editItem} onOpenChange={()=>setEditItem(null)}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>Edit Godown</DialogTitle></DialogHeader>
      ...
      <Dialog open={!!deleteItem} onOpenChange={()=>setDeleteItem(null)}><DialogContent className="max-w-sm"><DialogHeader><DialogTitle className="text-red-600">Delete Godown</DialogTitle></DialogHeader>
